...
x[U, V] ^ i2(W) = x[U, x[W, V]]
x[U, V] ^ a2(W) = x[U, x[V, W]]
...
import { genId, repr, wrapView } from "./util";
...
const BOX_TRANSFORMS: Record<BoxTransformType, BoxTransform> = {
  i1: (box, view) => ({
    ...box, first: { ...box, id: genId('box'), second: wrapView(view) }
  }),
  i2: (box, view) => ({
    ...box, second: { ...box, id: genId('box'), first: wrapView(view) }
  }),
  a1: (box, view) => ({
    ...box, first: { ...box, id: genId('box'), first: wrapView(view), second: box.first }
  }),
  a2: (box, view) => ({
    ...box, second: { ...box, id: genId('box'), second: wrapView(view), first: box.second }
  }),
  o1: (box, view) => ({
    ...box, orientation: rotated(box), first: wrapView(view), second: { ...box, id: genId('box') }
  }),
  o2: (box, view) => ({
    ...box, orientation: rotated(box), second: wrapView(view), first: { ...box, id: genId('box') }
  }),
  x1: (box, view) => ({
    ...box, first: { ...box, id: genId('box'), orientation: rotated(box), first: wrapView(view), second: box.first }
  }),
  x2: (box, view) => ({
    ...box, second: { ...box, id: genId('box'), orientation: rotated(box), first: wrapView(view) }
  }),
  y1: (box, view) => ({
    ...box, first: { ...box, id: genId('box'), orientation: rotated(box), second: wrapView(view) }
  }),
  y2: (box, view) => ({
    ...box, second: { ...box, id: genId('box'), orientation: rotated(box), first: box.second, second: wrapView(view) }
  }),
}
...
/**
 * Remove dead views and recursively clear empty containers.
 */
export function simplify(s: IBox | ITabs | null | undefined): IBox | ITabs | null {
  ...
  switch (s.type) {
    case "box":
      ...
      return (first && second) ? { ...s, first, second }
        : first ? first
          : second ? second
            : null
...
export function reducer(s: IBox | ITabs | null | undefined, action: BoxAction | ViewAction): IBox {
  s = reducer1(s, action)
  console.log('final', repr(s));
  return s
}

export function reducer1(s: IBox | ITabs | null | undefined, action: BoxAction | ViewAction): IBox {
  ...
  if (action.type !== 'kill' || action.simplify) {
    s = simplify(s)
    console.log('simplified', s);
  }

  if (!s)
    return { type: "box", id: "box0", orientation: "horizontal" }
  ...
  if (action.type === "kill") {
    if (s.type === 'tabs') {
      ...
      if (pos === -1)
        ...
        return { ...s, tabs: [...s.tabs.slice(0, pos), { ...s.tabs[pos], dead: true }, ...s.tabs.slice(pos + 1)] }
    } else if (s.type === "box") {
      return {
        ...s, first: reducer0(s.first, action), second: reducer0(s.second, action)
      }
    }
  }
  else {
